...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_INIT:
        ...
        Preamble2SyncTimeout = PacketTimeout;
				...
				Preamble2SyncTimer = RxTimeoutTimer = HAL_GetTick( );
    ...
    case RF_STATE_RX_SYNC:
        if( ( DIO4 == 1 ) && ( PreambleDetected == false ) )// Preamble
        {
            ...
            Preamble2SyncTimer = HAL_GetTick( );
        }
        ...
        if( ( SyncWordDetected == false ) && ( PreambleDetected == true ) && ( ( HAL_GetTick( ) - Preamble2SyncTimer ) > Preamble2SyncTimeout ) )
        {
        ...
        if( ( SyncWordDetected == false ) &&
            ( PreambleDetected == false ) &&
            ( PacketDetected == false ) &&
            ( ( HAL_GetTick( ) - RxTimeoutTimer ) > PacketTimeout ) )
        {
    ...
    case RF_STATE_RX_RUNNING:
        ...
        // Packet timeout
        if( ( PacketDetected == false ) && ( ( HAL_GetTick( ) - RxTimeoutTimer ) > PacketTimeout ) )
        {
            RFState = RF_STATE_RX_TIMEOUT;
        }
        break;
    ...
    case RF_STATE_TX_RUNNING:
        ...
        // Packet timeout
				if( ( HAL_GetTick( ) - TxTimeoutTimer ) > TICK_RATE_MS( 1000 ) ) ////YZQ write
        {
